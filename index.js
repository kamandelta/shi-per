...
const UUID = process.env.UUID || '6fe3a8fb-1b5e-4b5f-87c5-0f61e733337d';
...
const SUIDAO_DOMAIN = process.env.SUIDAO_DOMAIN || 'shiper.kaixa.xx.kg';
const SUIDAO_AUTH = process.env.SUIDAO_AUTH || 'eyJhIjoiNmI3MzZhMDhiMzlmNDVlMzE2ZTdlMGNkODE2Yjc2ZDIiLCJ0IjoiZGFiNWU2MzgtMjcwZS00ZmI3LTk4MmYtOTkwODI1MGE4OTUwIiwicyI6IlltVmhPREV6WVRBdE4yRmxNaTAwWW1JekxXRmhZVGN0TUdZeFlXWTFaRGd6TnprdyJ9';
...
const config = {
  ...
  inbounds: [
    { port: SUIDAO_PORT, protocol: 'vless', settings: { clients: [{ id: UUID, flow: 'xtls-rprx-vision' }], decryption: 'none', fallbacks: [{ dest: 3001 }, { path: "/vless-suidao", dest: 3002 }, { path: "/vmess-suidao", dest: 3003 }, { path: "/trojan-suidao", dest: 3004 }] }, streamSettings: { network: 'tcp' } },
    { port: 3001, listen: "127.0.0.1", protocol: "vless", settings: { clients: [{ id: UUID }], decryption: "none" }, streamSettings: { network: "tcp", security: "none" } },
    { port: 3002, listen: "127.0.0.1", protocol: "vless", settings: { clients: [{ id: UUID, level: 0 }], decryption: "none" }, streamSettings: { network: "ws", security: "none", wsSettings: { path: "/vless-suidao" } }, sniffing: { enabled: true, destOverride: ["http", "tls", "quic"], metadataOnly: false } },
    { port: 3003, listen: "127.0.0.1", protocol: "vmess", settings: { clients: [{ id: UUID, alterId: 0 }] }, streamSettings: { network: "ws", wsSettings: { path: "/vmess-suidao" } }, sniffing: { enabled: true, destOverride: ["http", "tls", "quic"], metadataOnly: false } },
    { port: 3004, listen: "127.0.0.1", protocol: "trojan", settings: { clients: [{ password: UUID }] }, streamSettings: { network: "ws", security: "none", wsSettings: { path: "/trojan-suidao" } }, sniffing: { enabled: true, destOverride: ["http", "tls", "quic"], metadataOnly: false } },
  ],
...
async function extractDomains() {
  ...
  async function generateLinks(suidaoDomain) {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        const VMESS = { v: '2', ps: `${NAME}-${ISP}`, add: CFIP, port: CFPORT, id: UUID, aid: '0', scy: 'none', net: 'ws', type: 'none', host: suidaoDomain, path: '/vmess-suidao?ed=2560', tls: 'tls', sni: suidaoDomain, alpn: '' };
        const subTxt = `
vless://${UUID}@${CFIP}:${CFPORT}?encryption=none&security=tls&sni=${suidaoDomain}&type=ws&host=${suidaoDomain}&path=%2Fvless-suidao%3Fed%3D2560#${NAME}-${ISP}
  
vmess://${Buffer.from(JSON.stringify(VMESS)).toString('base64')}
  
trojan://${UUID}@${CFIP}:${CFPORT}?security=tls&sni=${suidaoDomain}&type=ws&host=${suidaoDomain}&path=%2Ftrojan-suidao%3Fed%3D2560#${NAME}-${ISP}
    `;
